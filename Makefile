...
# Fetch the latest Git tag if CONNECT_VERSION is not provided
export PREVIEW_VERSION ?= $(shell git describe --tags `git rev-list --tags --max-count=1` | sed 's/^v//')
...
K8S_PREVIEW_DOCKERFILE ?= $(CURDIR)/contrib/images/connect.sidecar.preview.k8s.Dockerfile
HELM_CHART := $(CURDIR)/contrib/chart
SEMVER := $(shell echo $(PREVIEW_VERSION) | grep -oE '^[0-9]+\.[0-9]+\.[0-9]+')
TRANSFORMED_SEMVER := $(shell echo $(SEMVER) | tr '.' '-')
K8S_NAMESPACE := $(shell echo $(PROJECT_NAME)-v$(shell echo $(TRANSFORMED_SEMVER)))
...
# If docker-compose is not found, try to find docker compose (space)
ifeq ($(DOCKER_COMPOSE),)
    DOCKER_COMPOSE := $(shell docker compose version > /dev/null 2>&1 && echo "docker compose")
endif

# Docker Compose availability flag
DOCKER_COMPOSE_AVAILABLE := true
ifeq ($(DOCKER_COMPOSE),)
    DOCKER_COMPOSE_AVAILABLE := false
endif

# If neither docker-compose nor docker compose is found, proceed without them
ifeq ($(DOCKER_COMPOSE_AVAILABLE),false)
    $(warning "Neither docker-compose nor docker compose found. Skipping docker-compose related targets.")
endif
...
ifneq ($(DOCKER_COMPOSE_AVAILABLE),false)
start-all-dev:
...
check-k8s-deps:
	@command -v minikube >/dev/null 2>&1 || { echo "minikube is not installed. Exiting."; exit 1; }
	@command -v kubectl >/dev/null 2>&1 || { echo "kubectl is not installed. Exiting."; exit 1; }
	@command -v helm >/dev/null 2>&1 || { echo "helm is not installed. Exiting."; exit 1; }
	@command -v docker >/dev/null 2>&1 || { echo "docker is not installed. Exiting."; exit 1; }
	@echo "All dependencies are installed."

check-env-var:
	@echo "Checking if PREVIEW_VERSION is set..."
	@if [ -z "$(PREVIEW_VERSION)" ]; then \
		echo "Error: PREVIEW_VERSION environment variable is not set."; \
		exit 1; \
	fi
...
run-minikube:
	...
	@command -v minikube > /dev/null || (echo "Error: Minikube is not installed." && exit 1)
...
build-k8s-preview:
	...
	DOCKER_BUILDKIT=1 docker build --target connect --no-cache --build-arg TAG="$(SEMVER)" -t "$(PROJECT_NAME):$(SEMVER)" -f "$(K8S_PREVIEW_DOCKERFILE)" . && \
	DOCKER_BUILDKIT=1 docker build --target blockchain --build-arg TAG="$(SEMVER)" -t "blockchain:$(SEMVER)" -f "$(K8S_PREVIEW_DOCKERFILE)" .
	@if [ $$? -ne 0 ]; then \
		echo "Error: Docker build failed"; \
		exit 1; \
	else \
		echo "Connect docker image v$(SEMVER) has been built successfully"; \
	fi
...
deploy-k8s-preview:
	...
	helm upgrade --install "$(PROJECT_NAME)" "$(HELM_CHART)" \
		...
		--set appVersion="$(SEMVER)" \
		--set image.tag="$(SEMVER)"
	@if [ $$? -ne 0 ]; then \
		echo "Error: Failed to deploy Helm chart."; \
		exit 1; \
	fi
...
helm ls --namespace $(1) --filter "^$(PROJECT_NAME)$$" | grep -q "$(PROJECT_NAME)"
endef

# Uninstall Helm release from all matching namespaces
delete-all-preview-charts:
	@echo "Uninstalling Helm release $(PROJECT_NAME) from namespaces matching pattern '$(shell echo $(PROJECT_NAME)-v*)'..."
	@kubectl get namespaces --no-headers -o custom-columns=":metadata.name" | grep "^$(shell echo $(PROJECT_NAME)-v*)" | while read namespace; do \
		echo "Checking Helm release in namespace $(K8S_NAMESPACE)"; \
		if $(call HELM_RELEASE_EXISTS,$$namespace); then \
			echo "Uninstalling Helm release $(PROJECT_NAME) from namespace $$namespace"; \
			helm uninstall $(PROJECT_NAME) --namespace $$namespace; \
		else \
			echo "Helm release $(PROJECT_NAME) not found in namespace $$namespace. Skipping..."; \
		fi; \
	done

# Delete namespaces matching the pattern
delete-all-preview-ns:
	@echo "Deleting namespaces matching pattern '$(shell echo $(PROJECT_NAME)-v*)'"
	@kubectl get namespaces --no-headers -o custom-columns=":metadata.name" | grep "^$(shell echo $(PROJECT_NAME)-v*)" | while read namespace; do \
		echo "Deleting namespace $$namespace"; \
		kubectl delete namespace $$namespace; \
	done

# Prune Docker resources inside Minikube
minikube-prune:
	@echo "Pruning unused Docker resources in Minikube..."
	@minikube ssh -- docker system prune -f
	@echo "Docker system prune in Minikube completed"

# Main target: Uninstall Helm releases and delete namespaces
clean-all-preview: check-k8s-deps delete-all-preview-charts delete-all-preview-ns minikube-prune
	@echo "Previews clean up has been completed"

.PHONY: check-k8s-deps delete-all-preview-charts delete-all-preview-ns minikube-prune clean-all-preview
